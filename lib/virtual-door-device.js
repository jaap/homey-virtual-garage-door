...
module.exports = class VirtualDoorDevice extends Homey.Device {
  ...
  async request(direction, { viaCapability = false } = {}) {
    ...
    if (viaCapability && direction === 'close') {
      // HomeKitty caches a HomeKit-written value even when this app rejects
      // the write, so an Apple Home read right after a close tap can show
      // "Closed" before any sensor confirmed it. Re-emitting the real value
      // shortly after refreshes HomeKitty's cache and characteristics.
